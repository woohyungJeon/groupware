				...
				<table class="table_css">
					...
					<tbody>
						<c:forEach var="vReportDTO" items="${list }">
							<tr class="list_css"
								onclick="location.href='${pageContext.request.contextPath }/reportVacationReceptionView/${vReportDTO.seq_ew_draft }?pg=${pg }'">
...
<ul class= "paging_css">
			<c:if test="${startPage > 3}">
				<li><a class="paging_a" href="reportVacationReceptionList?pg=${startPage-1}">이전</a></li>
			</c:if>

			<c:forEach var="i" begin="${startPage}" end="${endPage}" step="1">
				<c:if test="${pg == i}">
					<li><a class="paging_a" href="reportVacationReceptionList?pg=${i}">${i}</a></li>
				</c:if>
			...
			<c:if test="${endPage < totalP}">
				<li><a class="paging_a" href="reportVacationReceptionList?pg=${endPage+1}">다음</a></li>
			</c:if>
		</div>
